...
public:
    ...
    bool detectLoopClosure(int *latestID, int *closestID)
    {
        ...
        //bb:search the last keyframe's nearest keyframes
        ...
        for (int i = 0; i < (int)pointSearchIndLoop.size(); ++i)
        {
            ...
            //bb: find the nearest keyframe whose time is not close
        ...
        *latestKeyFrameCloud += *transformPointCloud(surfCloudKeyFrames[latestFrameIDLoopCloure],   &cloudKeyPoses6D->points[latestFrameIDLoopCloure]);
        ...
        //bb: combine the time_close frame of the nearset frame
...
//        icp.setin
        ...
        float noiseScore = icp.getFitnessScore();//bb: noise right??
    ...
    //bb:search nearby key poses and downsample them and then extract the local map points
    void extractNearby()
    {
        pcl::PointCloud<PointType>::Ptr surroundingKeyPoses(new pcl::PointCloud<PointType>());
        pcl::PointCloud<PointType>::Ptr surroundingKeyPosesDS(new pcl::PointCloud<PointType>());//bb: the key poses after downsample
        std::vector<int> pointSearchInd;
        std::vector<float> pointSearchSqDis;

        // extract all the nearby key poses and downsample them
        kdtreeSurroundingKeyPoses->setInputCloud(cloudKeyPoses3D); // create kd-tree
        //bb: kd-tree is used for nearby key poses
    ...
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i)
        {
            if (pointDistance(cloudToExtract->points[i], cloudKeyPoses3D->back()) > surroundingKeyframeSearchRadius)
                continue;
            int thisKeyInd = (int)cloudToExtract->points[i].intensity;//bb: the index of this key frame
            //bb: tranform the corner points and surfpoints of the nearby keyFrames into the world frame
    ...
    void extractSurroundingKeyFrames()
    {
        ...
        if (loopClosureEnableFlag == true)//bb:TODO: a little weired: Loop closure should search the whole map while
        {
            extractForLoopClosure(); //bb: the name is misleading
        } else {
            extractNearby();
        }
    }
    ...
    bool saveFrame()
    {
        ...
        //bb: judge whther should generate key pose
    ...
    void addGPSFactor()
    {
        ...
        {
            //bb: the translation between the first and last pose is very small
            if (pointDistance(cloudKeyPoses3D->front(), cloudKeyPoses3D->back()) < 5.0)
                return;
        }

        // pose covariance small, no need to correct
        //bb: the odom covariance is small TODO: although small maybe some interpolation
