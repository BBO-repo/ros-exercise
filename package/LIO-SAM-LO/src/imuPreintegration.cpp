...
public:
    ...
    {
        //bb: addition
        subPoseOdomToMap = nh.subscribe<geometry_msgs::PoseStamped>("lio_sam_lo/mapping/pose_odomTo_map", 1,&IMUPreintegration::odomToMapPoseHandler,      this, ros::TransportHints().tcpNoDelay());
        //bb: addition

        subImu      = nh.subscribe<sensor_msgs::Imu>  (imuTopic,                   2000, &IMUPreintegration::imuHandler,      this, ros::TransportHints().tcpNoDelay());
        subOdometry = nh.subscribe<nav_msgs::Odometry>("lio_sam_lo/mapping/odometry", 5,    &IMUPreintegration::odometryHandler, this, ros::TransportHints().tcpNoDelay());

        pubImuOdometry = nh.advertise<nav_msgs::Odometry> (odomTopic, 2000);
        pubImuPath     = nh.advertise<nav_msgs::Path>     ("lio_sam_lo/imu/path", 1);
    ...
    //bb: the odometry computed by the mapoptimization module
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        //bb:                       state variable: pose velo  measurement
        gtsam::ImuFactor imu_factor(X(key - 1), V(key - 1), X(key), V(key), B(key - 1), preint_imu);
        graphFactors.add(imu_factor);
        // add imu bias between factor
        //bb: BetweenFactor illustrate the relationship between two state
        ...
        // repropogate
        //bb: to get more correct imu_odom
        if (!imuQueImu.empty())
        {
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        // publish static tf
        //bb: the tf of map relative to odom  Todo: something can be done here to do localizaing in built map
        //todo: noted by gc
        ...
        double imuTime = ROS_TIME(&thisImu);
        double dt = (lastImuT_imu < 0) ? (1.0 / 500.0) : (imuTime - lastImuT_imu);//bb: first IMU or parameter was reset
        ...
        pubImuOdometry.publish(odometry);//bb: publish the state predicted by IMu
